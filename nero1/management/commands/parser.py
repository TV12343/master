...
InnerBlock = namedtuple('BlockYoll', 'name2,review2,date2,rating2')
...
class BlockYoll(InnerBlock):
    ...
    def __str__(self):
        return f'{self.name2}\t{self.review2} {self.date2} {self.rating2}'
...
class ReviewParser:
    ...
    def parse_block_yoll(self, item):
        #print(item)
        name2_block = item.select_one('div.reviews__item-user')
        #print(name2_block)
    ...
    # парсинг отзыва
        review2_block = item.select_one('div.reviews__item-text')
        review2 = review2_block.text.strip()
    # парсинг рейтинга
        rating2_block = item.select_one('div.rating span')
        rating2 = rating2_block.text.strip()
    # парсинг даты
    ...
    def get_blocks(self, page: int = None):
        ...
        # Запрос CSS-селектора, состоящего из множества классов, производится через select
        container = soup.select('div.reviews__item')
        for item in container:
            block_yoll = self.parse_block_yoll(item=item)
            print(block_yoll)
